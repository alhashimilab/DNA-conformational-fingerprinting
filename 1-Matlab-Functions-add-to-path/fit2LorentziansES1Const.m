function [Fit2Lorentzians,resnorm,exitflag,output,Lorentz1_2,Lorentz2_2,p_WB_2L,p_ES1_2L,cs_WB_2L,cs_ES1_2L] = fit2LorentziansES1Const(x_data, y_data, initParams, lb, ub, x_fit)
...
[fit2Params,resnorm,resid,exitflag,output,lambda,Jacobian] = lsqcurvefit(F_2_lorentzians,initParams,x_data,y_data,lb,ub);
...
p_ES1_amp_2 = (fit2Params(1)/(fit2Params(1)+fit2Params(4)))*100;

amp_WB_2_err    = (ci(4,2)-ci(4,1))/2;
amp_ES1_2_err = (ci(1,2)-ci(1,1))/2;

p_WB_amp_2_err = p_WB_amp_2*sqrt((amp_WB_2_err/fit2Params(4))^2+...
((amp_ES1_2_err)^2+(amp_WB_2_err)^2)/((fit2Params(1)+fit2Params(4))^2));
p_ES1_amp_2_err = p_ES1_amp_2*sqrt((amp_ES1_2_err/fit2Params(1))^2+...
((amp_ES1_2_err)^2+(amp_WB_2_err)^2)/((fit2Params(1)+fit2Params(4))^2));
...
p_ES1_trapz_2 = (trapz1_2/(trapz1_2+trapz2_2))*100;
...
if p_ES1_amp_2_err > std([p_ES1_amp_2,p_ES1_trapz_2])
    p_ES1_2_err = p_ES1_amp_2_err;
else
    p_ES1_2_err = std([p_ES1_amp_2,p_ES1_trapz_2]);
end

p_WB_2L  = [mean([p_WB_amp_2,p_WB_trapz_2]),p_WB_2_err];
p_ES1_2L = [mean([p_ES1_amp_2,p_ES1_trapz_2]),p_ES1_2_err];

cs_WB_2L       = fit2Params(6);
cs_WB_2L_err   = (ci(6,2)-ci(6,1))/2;
cs_WB_2L       = [cs_WB_2L,cs_WB_2L_err];

cs_ES1_2L     = fit2Params(3);
cs_ES1_2L_err = (ci(3,2)-ci(3,1))/2;
cs_ES1_2L     = [cs_ES1_2L,cs_ES1_2L_err];
