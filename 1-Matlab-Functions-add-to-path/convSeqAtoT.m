function seq_con_T = convSeqAtoT(seq_con_A)

seq_cell_split   = cell(length(seq_con_A),3);
seq_cell_split_T = cell(length(seq_con_A),3);
seq_con_T        = cell(length(seq_con_A),1);

for i = 1:length(seq_con_A)
    
    seq_cell_split{i,1} = seq_con_A{i,1}(1);
    seq_cell_split{i,2} = seq_con_A{i,1}(2);
    seq_cell_split{i,3} = seq_con_A{i,1}(3);
